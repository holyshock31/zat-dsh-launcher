...
function renderLauncherUpdate(info) {
  ...
  const v = info || {}
  els.launcherVersion.textContent = `v${v.version || '1.0.0'}`
...
async function init() {
  ...
  if (els.launcherCheckUpdate) els.launcherCheckUpdate.onclick = async () => {
    ...
    try {
      const timeout = new Promise(resolve => setTimeout(() => resolve({ ok: false, checkFailed: true, version: '1.0.0' }), 12000))
...
async function loadState() {
  ...
  if (els.version) els.version.textContent = `v${s.version || '1.0.0'}`
  const tid = state.selectedTerminalId
